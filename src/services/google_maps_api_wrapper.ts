...
export class GoogleMapsAPIWrapper {
  ...
  constructor(
      _el: HTMLElement, latitude: number, longitude: number,
      private _zone: NgZone) {
    this._el = _el;
    this._map = new google.maps.Map(
        this._el, {center: {lat: latitude, lng: longitude}});
    this._createObservables();
  }

  createEventObservable<E>(
      eventName: string,
      callback: (observer: Rx.Observer<E>) => void): Observable<E> {
  ...
  private _createObservables() {
    this._centerChangeObservable =
        this.createEventObservable<google.maps.LatLngLiteral>(
            'center_changed',
            (observer: Rx.Observer<google.maps.LatLngLiteral>) => {
              const center = this._map.getCenter();
              observer.onNext({lat: center.lat(), lng: center.lng()});
            });
    this._zoomChangeObservable = this.createEventObservable<number>(
        'zoom_changed', (observer: Rx.Observer<number>) => {
   ...
   */
  createMarker(
      options: google.maps.MarkerOptions = <google.maps.MarkerOptions>{}):
      google.maps.Marker {
...
export class GoogleMapsAPIWrapperFactory {
  ...
  create(el: HTMLElement, latitude: number, longitude: number):
      GoogleMapsAPIWrapper {
    if (this._instance) {
      throw new Error('instance already created');
    }
    return this._instance =
               new GoogleMapsAPIWrapper(el, latitude, latitude, this._zone);
  }
