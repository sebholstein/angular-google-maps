...
export class AgmHeatmapLayer implements OnInit, OnChanges, OnDestroy{
  ...
  private static _heatmapLayerOptions: string[] =
    [ 'data', 'dissipating', 'gradient', 'maxIntensity', 'opacity', 'radius'];

  /**
   * The data points to display. Required.
   */
  @Input() data: google.maps.MVCArray<google.maps.LatLng|google.maps.visualization.WeightedLocation> | google.maps.LatLng[] | google.maps.visualization.WeightedLocation[] = null;
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    /* TODO: this needs to be simplified perhaps setting dirty flag or needsRender with ngDoCheck()
     * https://github.com/angular/angular/blob/master/packages/common/src/directives/ng_for_of.ts#L138-L156
     */
    let isData: boolean;
    let onlyData: boolean;
    for (let propName in changes) {
      if (propName === 'data') {
        if (changes[propName].currentValue !== changes[propName].previousValue) {isData = true; }
      } else {
        if (changes[propName].currentValue !== changes[propName].previousValue) {onlyData = true; }
      }
    }
    if (isData && onlyData) {
      this._updateData();
    } else {this._updatePolygonOptions(changes); }
  }
  ...
  private _updatePolygonOptions(changes: SimpleChanges) {
    ...
    if (Object.keys(options).length > 0) {
      this._manager.setOptions(this);
    }
  }

  private _updateData() {
    this._manager.setData(this, this.data);
  }
